...
parser.add_argument('--minimum_feature_count', '-m', type=int, action='store')
...
minimum_feature_count = int(args.minimum_feature_count)
...
def process_locus(out_tmp,root,chrom,left_bounds_chrom, right_bounds_chrom,start,end,splice_site_width,minimum_read_count,junctions,cutoff,abpoa,verbose,minimum_feature_count):
    ...
    peak_areas, toWrite_N_l = SpliceDefineConsensus.find_peaks(histo_left_bases[chrom], True, cutoff, histo_cov, 'l', peak_areas, chrom,csDict,start,end,splice_site_width,minimum_feature_count,junctions)
    peak_areas, toWrite_N_r = SpliceDefineConsensus.find_peaks(histo_right_bases[chrom], False, cutoff, histo_cov, 'r', peak_areas, chrom,csDict,start,end,splice_site_width,minimum_feature_count,junctions)
    ...
    seqDict=SpliceDefineConsensus.define_start_end_sites(start_end_dict, start_end_dict_mono,upstream_buffer,downstream_buffer,minimum_feature_count)
    ...
    for isoform,reads in seqDict.items():
        if len(reads)>=minimum_read_count:
...
def main():
    ...
    samples=set()
    for line in open(out_path+'/fastaFiles.info'):
        sample=line.strip().split('\t')[1]
        samples.add(sample)

    samples=list(samples)

    out_i2s=open(out_path+'/isoform2samples.txt','w')
    out_i2s.write('isoform\t' + '\t'.join(samples) + '\n')
    ...
    for root in roots:
        ...
        results[root]=pool.apply_async(process_locus,[out_tmp,root,chrom,left_bounds_sub[chrom], right_bounds_sub[chrom],start,end,splice_site_width,minimum_read_count,junctions,cutoff,abpoa,False,minimum_feature_count])

    total_roots=len(roots)
    pool.close()
    print(f'\t{len(results)} loci submitted for mutlithreading')
    ...
    while delay:
        ...
        for root in results:
            if results[root].ready():
                ...
                finished_root_names.add(root)
        for root in finished_root_names:
            results.pop(root)

        if finished_roots == 0:
            delay=False
        print(f'\t\tfinished {finished_roots} loci totalling {reCounter} putative, unfiltered isoforms. {len(results)} loci remaining',' '*30,end='\r')


    print(f'\n\t{len(results)} loci took too long to complete\n\tterminating multi-threading pool',' '*30)
    pool.terminate()
    pool.join()
    ...
    for root in unfinished_roots:
        ...
        left_bounds_sub,right_bounds_sub=SpliceDefineConsensus.prepare_locus(chrom,start,end,left_bounds,right_bounds)
        results[root]=process_locus(out_tmp,root,chrom,left_bounds_sub[chrom], right_bounds_sub[chrom],start,end,splice_site_width,minimum_feature_count,junctions,cutoff,abpoa,True,minimum_read_count)
